...
class AudioProcessor(object):
 ...
 #       self.mfcc_ = fbank(background_clamp,
 #           winlen=model_settings['window_size_samples'],
 #           winstep=model_settings['window_stride_samples'],
 #           nfilt=model_settings['dct_coefficient_count'],
 #           lowfreq=10,
 #           highfreq=4000,
 #           winfunc=np.hamming)
        ...
        self.mfcc_ = tf.log(tf.minimum(float(2e4),tf.maximum(self.mfcc_,1))) / 10.0
...
class GSCDataset(Dataset):
    def __init__(self, data_url, data_dir, silence_percentage, unknown_percentage,
        ...
        if self.model_architecture == "gru":
            self.data = self.data.reshape((-1, input_time_size, input_frequency_size))
            self.data = np.transpose(self.data, (0, 2, 1))  # (batch, sequnce, feature)
